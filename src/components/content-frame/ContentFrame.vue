...
<template>
  <div class="p-3 flex flex-col h-full">
    <div :class="{ 'border-b': !props.hideSplitBar }"
      ...
      <span v-if="props.showPaginationInTitle" class="font-bold" style="color: var(--secondary-text)">({{
        (props.currentTab ?? 0) + 1 }}/{{ props.totalTab }})</span>
    </div>
    <div class="flex-1 overflow-y-auto">
      <slot />
    </div>
    <div v-if="props.showPagination" class="flex justify-center items-center gap-2">
      <div v-for="index in props.totalTab" :key="index" :style="{
